...
5. **产出**：用 `python-pptx` 生成/改写 `outputs/ppt/deck-NN.pptx`（编辑模式 NN 递增、**不覆盖原文件**），另写 `outputs/ppt/deck-NN-meta.md`（编辑模式含逐条改动清单）。交付前校验可打开、无溢出/重叠。
...
| `knowledge/style-application.md` | 六风格在 PPT 的落地规格 | 内置 |
